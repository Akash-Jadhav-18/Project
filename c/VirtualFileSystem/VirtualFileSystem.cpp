...
typedef struct inode
{
	...
	int FileActualSize;
	int FileType;
	char *Buffer;  // char *Buffer[5]; Buffer is a one dimentional array which contains 5 elements each elemeny is of type char*.
	int LinkCount;
	int ReferenceCount;
	int permission;
	struct inode *next;
...
int main()
{
 ...
 else if(count == 2)
 {
	...
	else if(_stricmp(command[0],"write") == 0)
	{
		...
		if(ret == -2)
			printf("ERROR : There is no sufficient memory towrite\n");
